// src/components/shop/IncomingShopOrderModal.jsx
...
import { DEMO, getShopTotal } from '@/screens/main/shop/shopDemo';

const COUNTDOWN_SECONDS = 20;

export default function IncomingShopOrderModal({ visible, onAccept, onDecline, order }) {
        ...
        <AppModal visible={visible} onClose={handleDecline} closeOnOverlay={false} showHandle hideActions scrollable={false} className="pb-2">
            ...
            <View className="flex-row items-center justify-between mb-3">
                <View className="flex-1 mr-3">
                    <Text className="text-xl font-inter-bold text-foreground">Shop for Me Request</Text>
            ...
            {/* Item count + budget — the thing food/parcel/gig offers don't have */}
            <View className="flex-row items-center gap-3 rounded-xl border border-border bg-background-muted px-3 py-3 mb-3">
                <View className="h-11 w-11 items-center justify-center rounded-full bg-primary/15 border border-primary/30">
                    <Icon name="cart-outline" size={20} color={primaryHex} />
                </View>
                ...
                <Text className="text-sm font-inter-bold text-foreground">Budget ${job.budgetLimit?.toFixed(2)}</Text>
            </View>
            ...
            <View className="gap-2.5 mb-4">
                <View className="flex-row items-start gap-3 rounded-xl border border-border bg-background-muted px-4 py-3">
                    <View className="h-9 w-9 items-center justify-center rounded-full bg-primary/15 border border-primary/30">
                        <Icon name="storefront-outline" size={18} color={primaryHex} />
                    </View>
                    <View className="flex-1">
            ...
            <StatRow
                ...
                items={[
                    { label: 'To store', value: job.distanceToStore },
                    { label: 'To customer', value: job.distanceToCustomer },
